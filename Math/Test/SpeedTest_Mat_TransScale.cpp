...
#ifdef NDEBUG
#define	numMat_TransScale 1000000
#else
#define	numMat_TransScale 1
#endif

TEST(SpeedTest_Mat_TransScale, TestConfig::ALL)
{
#if SpeedTest_Mat_TransScale

	Trans A(3.0f, 5.0f, 8.0f);
	...
	*pOut2 = (*pA2) * (*pB2);
	...
	CHECK(pOut->m11() == 0.0f);
	CHECK(pOut->m12() == 6.0f);
	CHECK(pOut->m13() == 35.0f);
	CHECK(pOut->m14() == 32.0f);
	CHECK(eq_float(pOut->m15(), 1.0f));

	CHECK(pOut->m0() == pOut2->_m0);
	CHECK(pOut->m1() == pOut2->_m1);
	CHECK(pOut->m2() == pOut2->_m2);
	CHECK(pOut->m3() == pOut2->_m3);
	CHECK(pOut->m4() == pOut2->_m4);
	CHECK(pOut->m5() == pOut2->_m5);
	CHECK(pOut->m6() == pOut2->_m6);
	CHECK(pOut->m7() == pOut2->_m7);
	CHECK(pOut->m8() == pOut2->_m8);
	CHECK(pOut->m9() == pOut2->_m9);
	CHECK(pOut->m10() == pOut2->_m10);
	CHECK(pOut->m11() == pOut2->_m11);
	...
	tA0.Tic();
	for (int i = 0; i < numMat_TransScale; i++)
	{
		*pOut = (*pA) * (*pB);
	}
	tA0.Toc();
	...
	Trace::out2("SpeedTest_Mat_TransScale  ratio: %f   (1.25) \n", ratio );
	CHECK(ratio > 1.25f);
#endif
#endif
