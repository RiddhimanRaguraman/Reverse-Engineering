...
#ifdef NDEBUG
#define	numDETRotScale 10000000
#else
#define	numDETRotScale 1
#endif

TEST(SpeedTestDet_RotScale, TestConfig::ALL)
{
#if SpeedTestDet_RotScale
	...
	*pOut2 = pA2->det();
	...
	tA0.Tic();
	for (int i = 0; i < numDETRotScale; i++)
	{
		*pOut = pA->det();
	}
	tA0.Toc();
	...
	Trace::out2("SpeedTestDet_RotScale  ratio: %f   (1.25)\n", ratio	);
	CHECK(ratio > 1.25f);
#endif
#endif
