...
#ifdef NDEBUG
#define	numMat_ScaleMat4 1000000
#else
#define	numMat_ScaleMat4 1
#endif

TEST(SpeedTest_Mat_ScaleMat4, TestConfig::ALL)
{
#if SpeedTest_Mat_ScaleMat4
	...
	*pOut2 = (*pA2) * (*pB2);
	...
	CHECK(eq_float(pOut->m1(), 6.0f));
	CHECK(eq_float(pOut->m2(), 18.0f));
	CHECK(eq_float(pOut->m3(), 6.0f));
	CHECK(eq_float(pOut->m4(), 30.0f));
	CHECK(eq_float(pOut->m5(), 10.0f));
	CHECK(eq_float(pOut->m6(), 30.0f));
	CHECK(eq_float(pOut->m7(), 15.0f));
	CHECK(eq_float(pOut->m8(),  12.0f));
	CHECK(eq_float(pOut->m9(),  4.0f));
	CHECK(eq_float(pOut->m10(), 2.0f));
	CHECK(eq_float(pOut->m11(), 12.0f));
	...
	tA0.Tic();
	for (int i = 0; i < numMat_ScaleMat4; i++)
	{
		*pOut = (*pA) * (*pB);
	}
	tA0.Toc();
	...
	Trace::out2("SpeedTest_Mat_ScaleMat4  ratio: %f  (1.5) \n", ratio );
	CHECK(ratio > 1.5f);
#endif
#endif
