...
#ifdef NDEBUG
#define	numMat_Mat4Trans 1000000
#else
#define	numMat_Mat4Trans 1
#endif

TEST(SpeedTest_Mat_Mat4Trans, TestConfig::ALL)
{
#if SpeedTest_Mat_Mat4Trans
	...
	*pOut2 = (*pA2) * (*pB2);
	...
	CHECK(eq_float(pOut->m0(), 11.0f));
	CHECK(eq_float(pOut->m1(), 12.0f));
	CHECK(eq_float(pOut->m2(), 10.0f));
	CHECK(eq_float(pOut->m3(), 2.0f));
	CHECK(eq_float(pOut->m4(), 15.0f));
	CHECK(eq_float(pOut->m5(), 17.0f));
	CHECK(eq_float(pOut->m6(), 12.0f));
	CHECK(eq_float(pOut->m7(), 3.0f));
	CHECK(eq_float(pOut->m8(),  24.0f));
	CHECK(eq_float(pOut->m9(),  32.0f));
	CHECK(eq_float(pOut->m10(), 13.0f));
	CHECK(eq_float(pOut->m11(), 6.0f));
	CHECK(eq_float(pOut->m12(), 29.0f));
	CHECK(eq_float(pOut->m13(), 40.0f));
	CHECK(eq_float(pOut->m14(), 22.0f));
	...
	tA0.Tic();
	for (int i = 0; i < numMat_Mat4Trans; i++)
	{
		*pOut = (*pA) * (*pB);
	}
	tA0.Toc();
	...
	Trace::out2("SpeedTest_Mat_Mat4Trans  ratio: %f   (1.15) \n", ratio );
	CHECK(ratio > 1.15f);
#endif
#endif
