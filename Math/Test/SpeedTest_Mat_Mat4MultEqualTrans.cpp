...
#ifdef NDEBUG
#define	numMat_Mat4MultEqualTrans 1000000
#else
#define	numMat_Mat4MultEqualTrans 1
#endif

TEST(SpeedTest_Mat_Mat4MultEqualTrans, TestConfig::ALL)
{
#if SpeedTest_Mat_Mat4MultEqualTrans
	...
	*pA2 *= (*pB2);
	...
	CHECK(eq_float(pA->m0(), 9.0f));
	CHECK(eq_float(pA->m1(), 16.0f));
	CHECK(eq_float(pA->m2(), 18.0f));
	CHECK(eq_float(pA->m3(), 2.0f));
	CHECK(eq_float(pA->m4(), 12.0f));
	CHECK(eq_float(pA->m5(), 23.0f));
	CHECK(eq_float(pA->m6(), 24.0f));
	CHECK(eq_float(pA->m7(), 3.0f));
	CHECK(eq_float(pA->m8(), 18.0f));
	CHECK(eq_float(pA->m9(), 44.0f));
	CHECK(eq_float(pA->m10(), 37.0f));
	CHECK(eq_float(pA->m11(), 6.0f));
	CHECK(eq_float(pA->m12(), 22.0f));
	CHECK(eq_float(pA->m13(), 54.0f));
	CHECK(eq_float(pA->m14(), 50.0f));
	...
	tA0.Tic();
	for (int i = 0; i < numMat_Mat4MultEqualTrans; i++)
	{
		*pA *= (*pB);
	}
	tA0.Toc();
	...
	Trace::out2("SpeedTest_Mat_Mat4MultEqualTrans  ratio: %f   (1.5) \n", ratio );
	CHECK(ratio > 1.5f);
#endif
#endif
