...
#ifdef NDEBUG
#define	numMat_ScaleMultEqualScale 1000000
#else
#define	numMat_ScaleMultEqualScale 1
#endif

TEST(SpeedTest_Mat_ScaleMultEqualScale, TestConfig::ALL)
{
#if SpeedTest_Mat_ScaleMultEqualScale

	Scale A(3.0f, 5.0f, 2.0f);
	Scale A2(3.0f, 5.0f, 2.0f);
	...
	*pA2 *= (*pB2);
	...
	tA0.Tic();
	for (int i = 0; i < numMat_ScaleMultEqualScale; i++)
	{
		*pA *= (*pB);
	}
	tA0.Toc();
	...
	Trace::out2("SpeedTest_Mat_ScaleMultEqualScale  ratio: %f    (2.5) \n", ratio );
	CHECK(ratio > 2.5f);
#endif
#endif
