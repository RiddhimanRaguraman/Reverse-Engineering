...
#ifdef NDEBUG
#define	numMat_TransTrans 1000000
#else
#define	numMat_TransTrans 1
#endif

TEST(SpeedTest_Mat_TransTrans, TestConfig::ALL)
{
#if SpeedTest_Mat_TransTrans

	Trans A(3.0f, 5.0f, 6.0f);
	Trans B(2.0f, 7.0f, 3.0f);
	Mat4 C = A * B;
	Trans out;
	Mat4Test out2;
	...
	*pOut2 = (*pA2) * (*pB2);
	...
	tA0.Tic();
	for (int i = 0; i < numMat_TransTrans; i++)
	{
		*pOut = (*pA) * (*pB);
	}
	tA0.Toc();
