...
#ifdef NDEBUG
#define	numMat_TransMat4 1000000
#else
#define	numMat_TransMat4 1
#endif

TEST(SpeedTest_Mat_TransMat4, TestConfig::ALL)
{
#if SpeedTest_Mat_TransMat4
	...
	*pOut2 = (*pA2) * (*pB2);
	...
	CHECK(eq_float(pOut->m0(), 5.0f));
	CHECK(eq_float(pOut->m1(), 2.0f));
	CHECK(eq_float(pOut->m2(), 6.0f));
	CHECK(eq_float(pOut->m3(), 2.0f));
	CHECK(eq_float(pOut->m4(), 6.0f));
	CHECK(eq_float(pOut->m5(), 2.0f));
	CHECK(eq_float(pOut->m6(), 6.0f));
	CHECK(eq_float(pOut->m7(), 3.0f));
	CHECK(eq_float(pOut->m8(),  6.0f));
	CHECK(eq_float(pOut->m9(),  2.0f));
	CHECK(eq_float(pOut->m10(), 1.0f));
	CHECK(eq_float(pOut->m11(), 6.0f));
	CHECK(eq_float(pOut->m12(), 65.0f));
	CHECK(eq_float(pOut->m13(), 25.0f));
	CHECK(eq_float(pOut->m14(), 58.0f));
	CHECK(eq_float(pOut->m15(), 40.0f));
	...
	tA0.Tic();
	for (int i = 0; i < numMat_TransMat4; i++)
	{
		*pOut = (*pA) * (*pB);
	}
	tA0.Toc();
	...
	Trace::out2("SpeedTest_Mat_TransMat4  ratio: %f    (1.25)\n", ratio );
	CHECK(ratio > 1.25f);
#endif
#endif
