...
#ifdef NDEBUG
#define	numMat_Mat4Scale 1000000
#else
#define	numMat_Mat4Scale 1
#endif

TEST(SpeedTest_Mat_Mat4Scale, TestConfig::ALL)
{
#if SpeedTest_Mat_Mat4Scale
	...
	*pOut2 = (*pA2) * (*pB2);
	...
	CHECK(eq_float(pOut->m0(), 15.0f));
	CHECK(eq_float(pOut->m1(), 10.0f));
	CHECK(eq_float(pOut->m2(), 12.0f));
	CHECK(eq_float(pOut->m3(), 2.0f));
	CHECK(eq_float(pOut->m4(), 18.0f));
	CHECK(eq_float(pOut->m5(), 10.0f));
	CHECK(eq_float(pOut->m6(), 12.0f));
	CHECK(eq_float(pOut->m7(), 3.0f));
	CHECK(eq_float(pOut->m8(),  18.0f));
	CHECK(eq_float(pOut->m9(),  10.0f));
	CHECK(eq_float(pOut->m10(), 2.0f));
	CHECK(eq_float(pOut->m11(), 6.0f));
	CHECK(eq_float(pOut->m12(), 24.0f));
	CHECK(eq_float(pOut->m13(), 25.0f));
	CHECK(eq_float(pOut->m14(), 16.0f));
	...
	tA0.Tic();
	for (int i = 0; i < numMat_Mat4Scale; i++)
	{
		*pOut = (*pA) * (*pB);
	}
	tA0.Toc();
	...
	Trace::out2("SpeedTest_Mat_Mat4Scale  ratio: %f   (1.15) \n", ratio );
	CHECK(ratio > 1.15f);
#endif
#endif
