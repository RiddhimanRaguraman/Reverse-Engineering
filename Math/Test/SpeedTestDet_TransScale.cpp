...
#ifdef NDEBUG
#define	numDETTransScale 10000000
#else
#define	numDETTransScale 1
#endif

TEST(SpeedTestDet_TransScale, TestConfig::ALL)
{
#if SpeedTestDet_TransScale

	Scale B(3.0f, 5.0f, 2.0f);
	Trans A(2.0f, 7.0f, 3.0f);
	Mat4 C = A * B;
	volatile float out;
	volatile float out2;
	...
	*pOut2 = pA2->det();
	...
	tA0.Tic();
	for (int i = 0; i < numDETTransScale; i++)
	{
		*pOut = pA->det();
	}
	tA0.Toc();
