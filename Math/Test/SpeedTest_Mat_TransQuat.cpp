...
#ifdef NDEBUG
#define	numMat_TransQuat 1000000
#else
#define	numMat_TransQuat 1
#endif

TEST(SpeedTest_Mat_TransQuat, TestConfig::ALL)
{
#if SpeedTest_Mat_TransQuat
	...
	*pOut2 = (*pA2) * Mat4Test(*pB2);
	...
	CHECK(eq_float(pOut->m0(), 0.4063f));
	CHECK(eq_float(pOut->m1(), 0.0016f));
	CHECK(eq_float(pOut->m2(), 0.9137f));
	CHECK(eq_float(pOut->m3(), 0.0f));
	CHECK(eq_float(pOut->m4(), -0.5225f));
	CHECK(eq_float(pOut->m5(), 0.8207f));
	CHECK(eq_float(pOut->m6(), 0.2308f));
	CHECK(eq_float(pOut->m7(), 0.0f));
	CHECK(eq_float(pOut->m8(), -0.7495f));
	CHECK(eq_float(pOut->m9(), -0.5713f));
	CHECK(eq_float(pOut->m10(), 0.3343f));
	CHECK(eq_float(pOut->m11(), 0.0f));
	CHECK(eq_float(pOut->m12(), -2.8930f));
	CHECK(eq_float(pOut->m13(), 2.9659f));
	CHECK(eq_float(pOut->m14(), 4.5643f));
	...
	tA0.Tic();
	for (int i = 0; i < numMat_TransQuat; i++)
	{
		*pOut = (*pA) * (*pB);
	}
	tA0.Toc();
	...
	Trace::out2("SpeedTest_Mat_TransQuat  ratio: %f   (1.25) \n", ratio);
	CHECK(ratio > 1.25f);
#endif
#endif
