...
#ifdef NDEBUG
#define	numMat_QuatTrans 1000000
#else
#define	numMat_QuatTrans 1
#endif

TEST(SpeedTest_Mat_QuatTrans, TestConfig::ALL)
{
#if SpeedTest_Mat_QuatTrans
	...
	*pOut2 = (*pA2) * (*pB2);
	...
	CHECK(eq_float(pOut->m12(), 3.0f));
	CHECK(eq_float(pOut->m13(), 5.0f));
	CHECK(eq_float(pOut->m14(), 2.0f));
	...
	tA0.Tic();
	for (int i = 0; i < numMat_QuatTrans; i++)
	{
		*pOut = (*pA) * (*pB);
	}
	tA0.Toc();
	...
	Trace::out2("SpeedTest_Mat_QuatTrans  ratio: %f   (1.25)\n", ratio );
	CHECK(ratio > 1.25f);
#endif
#endif
